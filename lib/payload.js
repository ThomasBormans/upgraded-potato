...
var messages = {
	...
	default: {
		status: 501,
		key: 'err',
		message: 'Payload type not implemented. Did you use the correct type?'
	}
};
...
module.exports = function(res, type, options) {
	...
	// Set message to payload as default
	var payload = config.message;

	// If config has a key, create an object
	if(config.key) {
		payload = {};
		payload[config.key] = config.message;
	}

	res.status(config.status).json(payload);
};
